# -*- coding: utf-8 -*-
"""
Created on Tue Sep  8 06:28:24 2020
...
artiq_templates = { "DDS": {
                        ...
                        "initialize": ["self.urukul{board_id}_ch{channel_id}.init()",
                                       "self.urukul{board_id}_cpld.cfg_sw({channel_id}, True)",
                                       "self.urukul{board_id}_ch{channel_id}.set_att({1.0})"
                                ],
                        "set_frequency": ["self.urukul{board_id}_ch{channel_id}.set({frequency}*{units}, amplitude = {amplitude})"],
                        "set_attenuation": ["self.urukul{board_id}_ch{channel_id}.set_att({attenuation})"],
                        "ramp_frequency": {
                                "control": "for i in range(0, {steps}):",
                                "block": ["self.urukul{board_id}_ch{channel_id}.set({frequency_per_step}*{i}*{freq_unit}, amplitude = {amplitude})",
                     ...
                     "DAC":{
                        ...
                        "initialize": ["self.zotino{board_id}.init()"],
                        "constant_voltage": ["self.zotino{board_id}.write_dac({channel_id}, {voltage})"],
                     ...
                     "ADC":{
                        ...
                        "initialize": {
                                ...
                                "block": ["self.sampler0.set_gain_mu(i, {channel_gain})",
                                         "delay(100*us)"],
                                "main": ["sampler_data{channel_id} = [0.0]*{number_of_channels}"]
                             },
                        "acquire_once": ["self.sampler{board_id}(sampler_data{board_id})"],
                        "acquire_continuous" : {
                                "main": ["{data_name} = []"],
                                "control": "for i in range(0, {steps})",
