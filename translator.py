...
class Block:
    ...
    def generate_output(self, end_in_newline=True) -> str:
        ...
        if self.control:
            compound_statement.insert(0, self.control_indented)

        print(self.lines_indented)
        self.__output = '\n'.join(self.lines_indented)
...
class Parser():
    ...
    def create_experiment(self):
        ...
        for group in self.logic:
            ...
            event_full = self.control + event_type
            event = Experiment.build(event_full,
                                     group['alias'],
                                     self.aliases,
                                     **group)
            this_experiment.add(event)

        return this_experiment

if __name__ == "__main__":
    # test_parse = Parser("first_sequence.json")
    # check = ArtiqBuild(test_parse.aliases)
    # print(str(check))
    # asd = test_parse.experiment
    # dfsf = test_parse.logic
    # asdasd = test_parse.create_experiment()
    # sequence = asdasd.sequence
    with open("first_sequence.json", 'r') as json_file:
        experiment_dictionary = json.load(json_file)

    aliases = experiment_dictionary["aliases"]
    AOM = ArtiqEvent("aom_2dmot", "set_frequency", "12345", aliases, frequency=70, amplitude=1.0, units="Hz")
    DAC = ArtiqEvent("aom_zotino", "set_voltage", "123324", aliases, voltage=4.0)

    # AnotherTest = ArtiqBuild(aliases)
    # print(AnotherTest)
    first_experiment = Experiment("test", "Artiq", aliases)
    first_experiment.add(AOM)
    first_experiment.add(DAC)

    print(first_experiment.translate())
